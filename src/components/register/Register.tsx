...
const Register = () => {
    ...
    <>
		...
		{step == 2 && (
			<form 
				className={
					`shadow-lg rounded-xl p-6 w-[90vw] md:w-1/2 bg-white flex gap-4 flex-col border
					${role == 'PROFESIONAL' ? 'border-purple-200/50' : 'border-blue-300'}`
				}
			>
				<ArrowLeft 
					className={
						`cursor-pointer 
						${role == 'PROFESIONAL' ? 'text-purple-500 hover:text-purple-700' : 'text-blue-500 hover:text-blue-800'} transition-colors`
					} 
				...
				<PersonalInfo 
					...
					textColor={`${role == 'PROFESIONAL' ? 'text-purple-700' : 'text-blue-500'}`} 
					borderColor={`${role == 'PROFESIONAL' ? 'border-purple-300' : 'border-blue-400'}`}
				/>
